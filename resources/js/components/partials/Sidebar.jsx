...
import { MENU_SIDEBAR } from '../constan/menu';
    ...
    <div>
      ...
      >
        ...
        <div className="space-y-8">
          ...
          <div>
            {/* <h3 className="text-xs uppercase text-slate-500 font-semibold pl-3">
              <span className="block lg:sidebar-expanded:hidden 2xl:hidden text-center w-6">
                •••
              </span>
              <span className="lg:hidden lg:sidebar-expanded:block 2xl:block">Pages</span>
            </h3> */}
            <ul className="mt-3">
              {MENU_SIDEBAR.map((row, idx) => (
                <li key={idx}>
                  <NavLink
                    end
                    to={row.uri}
                    className={({ isActive }) =>
                      'my-2 block text-slate-900 hover:text-slate-200 transition duration-150 truncate ' + (isActive ? '!text-indigo-500' : '')
                    }
                  >
                    <div className="flex items-center">
                      {/* <i className="text-2xl"><MdMoveToInbox/></i> */}
                      <span className="text-sm font-medium ml-3">{row.label}</span>
                    </div>
                  </NavLink>
                </li>
              ))}
